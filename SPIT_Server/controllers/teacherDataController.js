...
exports.deleteCourses = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const det = await Faculty.updateOne({ email: email }, faculty);
...
exports.addEvent = asyncHandler(async (req, res) => {
  ...
  try {
    const det = await Faculty.updateOne(
      { email: email },
      {
        $push: {
          events: events,
...
exports.getAllFacultyfullinfo = asyncHandler(async (req, res) => {
  try {
    const fullinfo = await Faculty.find({});
...
exports.checkFaculty = async (req, res) => {
  ...
  try {
    ...
    if (faculty) {
      ...
      if (appraisal.isSubmitted && !appraisal.HODReviewed)
        return res.status(200).json("Faculty has submmitted the appraisal");
      else
        return res.status(400).json("Faculty has not submmitted the appraisal");
    } else {
...
exports.getAllDepartments = async (req, res) => {
  try {
    ...
    for (var i = 0; i < faculty.length; i++) {
      if (
        faculty[i].department === "Principal" ||
        faculty[i].department === "Training and Placement Office" ||
        faculty[i].department === "Library"
      )
        continue;
